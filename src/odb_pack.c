 ...
 *   |
 *   |-# git_multipack_index_entry_find
...
static int pack_entry_find(struct git_pack_entry *e, struct pack_backend *backend, const git_oid *oid)
{
	...
	git_multipack_entry multipack_entry;
	size_t i;

	if (backend->midx &&
		git_multipack_index_entry_find(&multipack_entry, backend->midx, oid, GIT_OID_HEXSZ) == 0 &&
		multipack_entry.pack_index < git_vector_length(&backend->midx_packs)) {
		e->offset = multipack_entry.offset;
		git_oid_cpy(&e->sha1, &multipack_entry.sha1);
		e->p = git_vector_get(&backend->midx_packs, multipack_entry.pack_index);
		return 0;
	}
	...
	size_t len)
{
	...
	if (backend->midx) {
		error = git_multipack_index_entry_find(&multipack_entry, backend->midx, short_oid, len);
		if (error == GIT_EAMBIGUOUS)
			return error;
		if (!error && multipack_entry.pack_index < git_vector_length(&backend->midx_packs)) {
			e->offset = multipack_entry.offset;
			git_oid_cpy(&e->sha1, &multipack_entry.sha1);
			e->p = git_vector_get(&backend->midx_packs, multipack_entry.pack_index);
...
static int remove_multi_pack_index(struct pack_backend *backend)
{
	...
	git_multipack_index_free(backend->midx);
	backend->midx = NULL;
		...
		const char *packfile_name)
{
	...
	/* This is ensured by multipack_index_parse_packfile_name() */
...
static int refresh_multi_pack_index(struct pack_backend *backend)
{
	 ...
	 * old multipack index and move all the packfiles to the unindexed
	 ...
	 */
	if (backend->midx) {
		if (!git_multipack_index_needs_refresh(backend->midx, git_buf_cstr(&midx_path))) {
	...
	error = git_multipack_index_open(&backend->midx, git_buf_cstr(&midx_path));
	git_buf_dispose(&midx_path);
	if (error < 0)
		return error;
...
static int pack_backend__foreach(git_odb_backend *_backend, git_odb_foreach_cb cb, void *data)
{
	...
	if (backend->midx && (error = git_multipack_index_foreach_entry(backend->midx, cb, data)) != 0)
		return error;
	git_vector_foreach(&backend->packs, i, p) {
...
static int pack_backend__writemultipackindex(git_odb_backend *_backend)
{
	...
	error = git_multipack_index_writer_new(&w, backend->pack_folder);
	if (error < 0)
		return error;
	...
	git_vector_foreach(&backend->midx_packs, i, p) {
		...
		if (error < 0)
			goto cleanup;
		error = git_multipack_index_writer_add(w, git_buf_cstr(&idx_path));
	...
	/*
	 * Invalidate the previous midx before writing the new one. This is because
	 * the underlying file is mmap(2)-ed, and that would corrupt the
	 * previously-loaded contents.
	 */
	error = remove_multi_pack_index(backend);
	if (error < 0)
		goto cleanup;
	error = git_multipack_index_writer_commit(w);
	if (error < 0)
		goto cleanup;
