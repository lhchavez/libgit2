...
{
	git_multipack_index_file idx = {{0}};
	...
	if (git_multipack_index_parse(&idx, (const unsigned char *)git_buf_cstr(&midx_buf), git_buf_len(&midx_buf)) < 0)
		goto cleanup;

	/* Search for any oid, just to exercise that codepath. */
	if (git_multipack_index_entry_find(&e, &idx, &oid, GIT_OID_HEXSZ) < 0)
		goto cleanup;
